...
public class CameraControls extends RotatableLayout {
    ...
    public void onLayout(boolean changed, int l, int t, int r, int b) {
        ...
        switch (rotation) {
            case 90:
                expandedShutter.top = mSwitcher.getMeasuredWidth();
                expandedShutter.bottom = h - mPreview.getMeasuredWidth();
                break;
            case 180:
                expandedShutter.left = mSwitcher.getMeasuredWidth();
                expandedShutter.right = w - mPreview.getMeasuredWidth();
                break;
            case 270:
                expandedShutter.top = mPreview.getMeasuredWidth();
                expandedShutter.bottom = h - mSwitcher.getMeasuredWidth();
                break;
            default:
                expandedShutter.left = mPreview.getMeasuredWidth();
                expandedShutter.right = w - mSwitcher.getMeasuredWidth();
                break;
        }
    ...
    private void asRow(boolean top, int w, int h, int rotation, View...views) {
        ...
        int cellHeight = Math.max(largestHeight, cellWidth);
        ...
        for (int i = 0; i < views.length; i++) {
            ...
            switch (rotation) {
                case 90:
                    cx = (top ? cellHeight / 2 : usedHeight - cellHeight / 2) + mTopMargin;
                    cy = usedWidth - leftOffset - cellWidth * i - (cellWidth / 2);
                    break;
                case 270:
                    cx = top ? usedHeight - mTopMargin - cellHeight / 2 : cellHeight / 2;
                    cy = leftOffset + cellWidth * i + (cellWidth / 2);
                    break;
                case 180:
                    cx = usedWidth - leftOffset - cellWidth * i - (cellWidth / 2);
                    cy = top ? usedHeight - mTopMargin - cellHeight / 2 : cellHeight / 2;
                    break;
                default:
                    cx = leftOffset + cellWidth * i + (cellWidth / 2);
                    cy = (top ? cellHeight / 2 : usedHeight - cellHeight / 2) + mTopMargin;
                    break;
            }
