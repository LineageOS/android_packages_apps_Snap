...
public class CameraControls extends RotatableLayout {
    ...
    public void onLayout(boolean changed, int l, int t, int r, int b) {
        ...
        expandedShutter.left = Math.max(mIndicators.getMeasuredWidth(), mPreview.getMeasuredWidth());
    ...
    private void asRow(boolean top, int w, int h, int rotation, View...views) {
        int cellWidth = w / views.length;
        int leftOffset = (w - cellWidth * views.length) / 2;
        int largestHeight = 0;
        for (View v : views) {
        ...
        for (int i = 0; i < views.length; i++) {
            View v = views[i];
            int cx = leftOffset + cellWidth * i + (cellWidth / 2);
            int cy = top ? largestHeight / 2 : h - largestHeight / 2;
